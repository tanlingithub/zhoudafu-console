...
import {backCategoryPropertyNotRelation} from '@/const/api.js'
import TreeDate from '@/utils/tree-data'
import {postPropertyBatch} from '@/service/xpaas-commodity-center'
...
export default {
  ...
  data() {
    const currentUrl = backCategoryPropertyNotRelation({id: this.categoryId})
    return {
      currentUrl,
      tableConfig: {
        url: currentUrl,
  ...
  watch: {
    ...
    categoryId() {
      this.$nextTick(_ => {
        this.tableConfig.url = backCategoryPropertyNotRelation({
          id: this.categoryId
        })
      })
    }
